...
ArrayList<JSONObject> vectorRoot(string line){
    ...
    while (i < line.length()) {
        ...
        string ek = expectingKey?"true":"false";

        //cout<<"Expecting Key: " << ek<<endl;
        //cout<<"Last String: " << currentString<<endl;
        ...
        //if not in number or in string
            ...
            switch (c) {
                ...
                case '}': {
                    ...
                    //if the current bracket is the last bracket of the bracket stack, return the object. Cannot read in the leftover data after this point
                    if (bracketStack.size() == 1) {
                        rootList.add(rootList.size(),newObject);
                    }
                    ...
                    //if top of bracket stack is an object bracket
                        ...
                        if (objectStack.size() == 0) {
                            //cout << "EARLY RETURN: stack empty, objectKey=" << objectKey << endl;
                            cout<<"Error. Object Stack Depleted in Object Root"<<endl;
                        }
                ...
                case ']': {
                    ...
                    if (bracketStack.size() == 0) {
                        return rootList.toArrayList<JSONObject>();
                    }
                    ...
                    //if the top of bracket stack is an opening object bracket
                        ...
                        if (bracketStack.size() == 0) {
                            return l.toArrayList<JSONObject>();
                        }
                ...
                case ':':
                    ...
                    try {
                        if (expectingKey) {
                            throw runtime_error("Caught unexpected key");
                        }
                    }catch (runtime_error& e) {
                        cout << e.what() << endl;
                    }
    ...
    return listStack.pop().toArrayList<JSONObject>();

}

int main() {
    string line = readFile("/home/jacksonryan/CLionProjects/JSONProject/cmake-build-debug/Resources/small_array.json");
    //JSONObject object = objectRoot(line);

    ArrayList<JSONObject> list = vectorRoot(line);
    ...
    cout << list.getFirst().getFields().size() << endl;

    for (int i = 0; i < list.getFirst().getFields().size(); i++) {
        cout << list.getFirst().getFields().get(i).getName() << endl;
    }

    cout << endl << list.get(1).getFields().size() << endl;

    for (int i = 0; i < list.get(1).getFields().size(); i++) {
        cout << list.get(1).getFields().get(i).getName() << endl;
    }

    cout << endl << list.get(2).getFields().size() << endl;

    for (int i = 0; i < list.get(2).getFields().size(); i++) {
        cout << list.get(2).getFields().get(i).getName() << endl;
    }
